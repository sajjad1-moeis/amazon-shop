...
import { Add, ArrowSwapHorizontal, Trash, Edit2 } from "iconsax-reactjs";
...
export default function SearchSynonymsPage() {
  ...
  const load = useCallback(async () => {
    try {
      setLoading(true);
      const data = await adminSearchManagementService.listSynonyms();
  ...
  const openEdit = (r) => {
    setEditId(r.id);
    setCanonicalTerm(r.canonicalTerm ?? "");
    setSynonymTerm(r.synonymTerm ?? "");
  ...
  const save = async () => {
    try {
      setSaving(true);
      if (editId == null) {
        await adminSearchManagementService.createSynonym({
          canonicalTerm: canonicalTerm.trim(),
          synonymTerm: synonymTerm.trim(),
          isActive,
        });
        toast.success("مترادف ثبت شد");
      } else {
        await adminSearchManagementService.updateSynonym(editId, {
          canonicalTerm: canonicalTerm.trim(),
          synonymTerm: synonymTerm.trim(),
          isActive,
        });
  ...
  const remove = async (id) => {
    ...
    try {
      await adminSearchManagementService.deleteSynonym(id);
  ...
  return (
    <div className="space-y-6 pb-8">
      <AdminPageHeader
        title="مترادف‌های جستجو"
        subtitle="SynonymTerm به CanonicalTerm نگاشت می‌شود (تطبیق بدون حساسیت به حروف)"
          ...
          <Table>
            <TableHeader>
              <TableRow className="border-gray-600/60 hover:bg-transparent">
                <TableHead className="text-right text-gray-300">کانونیکال</TableHead>
                <TableHead className="text-right text-gray-300">مترادف</TableHead>
            ...
            <TableBody>
              {rows.map((r) => (
                <TableRow key={r.id} className="border-gray-600/50">
                  <TableCell className="font-medium text-white">{r.canonicalTerm}</TableCell>
                  <TableCell className="text-gray-300">{r.synonymTerm}</TableCell>
      ...
      <Dialog open={open} onOpenChange={setOpen}>
        <DialogContent className="border-gray-600 bg-gray-900 text-white sm:max-w-md">
          <DialogHeader>
            <DialogTitle>{editId == null ? "مترادف جدید" : "ویرایش مترادف"}</DialogTitle>
          </DialogHeader>
          <div className="space-y-4 py-2">
            <div className="space-y-2">
              <Label className="text-gray-300">عبارت کانونیکال</Label>
              <Input
                value={canonicalTerm}
            ...
            <div className="space-y-2">
              <Label className="text-gray-300">مترادف کاربر</Label>
              <Input
                value={synonymTerm}
            ...
            <div className="flex items-center gap-2">
              <Switch checked={isActive} onCheckedChange={setIsActive} id="syn-active" />
              <Label htmlFor="syn-active" className="text-gray-300">
                فعال
              </Label>
            </div>
          </div>
