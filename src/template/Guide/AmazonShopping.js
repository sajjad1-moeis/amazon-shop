export const amazonSteps = [
  {
    ...
    desc: `وارد <a href="https://amazon.ae" className="text-[#9D7600] font-medium hover:underline">amazon.ae</a> شوید، 
  ...
  },
  {
    ...
    desc: `لینک محصول را در بخش 
           <a href="/request-amazon-uae" className="text-[#9D7600] font-medium hover:underline">درخواست خرید از آمازون امارات</a>
    ...
    <div className="space-y-2 max-md:text-sm">
      <div class="space-y-2 text-gray-600 dark:text-dark-text">
        <p>
          ...
          به‌صورت مستقیم همیشه آسان نیست.
        </p>
        <p>
          ...
          <div className="space-y-6">
            {amazonSteps.map((item) => (
              <div key={item.id} className="flex items-start gap-4">
                <div className="w-8 h-8 flex-none dark:bg-dark-primary bg-[#9D7600] text-white rounded-full flex items-center justify-center text-sm">
                  {item.id}
                </div>
                <div>
                  <p className="font-semibold dark:text-dark-titre text-gray-600 text-base md:text-lg">{item.title}</p>
                  <p
                    dangerouslySetInnerHTML={{ __html: item.desc }}
                    className="text-gray-500 text-sm dark:text-dark-text mt-2"
                  />
                </div>
              </div>
            ))}
      ...
      <div className="!mt-8 md:mt-12 pt-8 md:pt-12 border-t-2 border-gray-200 dark:border-dark-field">
        ...
        <div className="space-y-3 mt-8">
          ...
          <ul className="space-y-2 text-gray-600 list-disc list-inside mt-3 max-md:text-sm dark:text-dark-text">
            <li>
              بخش Reviews و Ratings را بررسی کنید.{" "}
              <span className="text-primary-500 dark:text-dark-title">امتیاز بالاتر از ۴</span> و تعداد نظر زیاد نشانه
              خوبی است.
            </li>
            <li>
