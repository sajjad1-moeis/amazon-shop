...
export default function ComparisonTable({ products, features }) {
  ...
  const renderFeatureValue = (value) => {
    ...
    return <span className="text-sm text-gray-900 dark:text-dark-title">{value || "-"}</span>;
  };
        ...
        <table className="w-full">
          <thead>
            <tr className="border-b border-gray-200 dark:border-dark-stroke">
              <th className="text-right py-4 px-6 bg-gray-50 dark:bg-dark-bg/50 text-sm font-semibold text-gray-900 dark:text-dark-title">
                ویژگی
              </th>
              {products.map((product, index) => (
                <th
                  key={product.id}
                  className={cn(
                    "text-center py-4 px-6 bg-gray-50 dark:bg-dark-bg/50 text-sm font-semibold text-gray-900 dark:text-dark-title",
                ...
                {products.map((product, productIndex) => (
                  <td
                    ...
                    className={cn(
                      ...
                      productIndex === 0 && "border-r border-gray-200 dark:border-dark-stroke"
                    )}
                  >
