...
export default function ComparisonCard({ comparison, onDelete, onView }) {
  ...
  return (
    <div className="bg-white dark:bg-dark-box border border-gray-200 dark:border-0 rounded-xl p-2.5 sm:p-3">
      ...
      <div className="grid grid-cols-2 gap-2 sm:gap-3 mb-3 sm:mb-5 text-center">
        {/* Products Count */}
        <div className="bg-gray-100 dark:bg-dark-field rounded-lg p-1.5 sm:p-2">
          <div className="text-xs sm:text-sm font-medium text-gray-900 dark:text-dark-titre">
            {comparison.productsCount} محصول
          </div>
          <div className="text-[10px] sm:text-xs text-gray-600 dark:text-caption mt-1 sm:mt-2">تعداد محصولات</div>
        </div>
        <div className="bg-gray-100 dark:bg-dark-field rounded-lg p-1.5 sm:p-2">
          <div className="text-xs sm:text-sm font-medium text-gray-700 dark:text-dark-titre">{comparison.saveDate}</div>
          <div className="text-[10px] sm:text-xs text-gray-600 dark:text-caption mt-1 sm:mt-2">تاریخ ذخیره</div>
        </div>
      </div>
      ...
      <div className="flex gap-1.5 sm:gap-2">
        <Button
          variant="ghost"
          onClick={onDelete}
          className="text-red-600 flex-1 bg-gray-200 hover:text-red-700 rounded-lg hover:bg-red-50 dark:bg-dark-stroke dark:text-red-400 dark:hover:bg-red-900/20 border-red-300 dark:border-red-800 gap-1 sm:gap-2 text-xs sm:text-sm"
        >
          حذف
        </Button>
        <Button
