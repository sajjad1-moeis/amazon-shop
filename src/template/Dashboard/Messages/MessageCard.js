...
export default function MessageCard({ message }) {
  const getTypeBadge = (type) => {
    switch (type) {
      case "support":
        return (
          <span className="inline-flex items-center px-3 py-1 rounded-lg text-xs font-medium bg-purple-100 text-purple-700 dark:bg-purple-900/30 dark:text-purple-400">
            {message.typeLabel}
          </span>
        );
      case "payment":
  ...
  return (
    <div className="border-b border-gray-200 dark:border-dark-stroke last:border-b-0 pb-4 last:pb-0">
      <Accordion
        type="single"
        collapsible
        defaultValue={message.id === 3 ? `message-${message.id}` : undefined}
      >
        <AccordionItem value={`message-${message.id}`} className="border-0">
          <AccordionTrigger className="hover:no-underline py-2 [&>svg]:hidden group">
            <div className="flex items-center justify-between w-full">
              ...
              <div className="flex items-center gap-3 flex-1">
                ...
                />
                <h3 className="text-base font-semibold text-gray-900 dark:text-dark-title text-right">
                  {message.title}
                </h3>
              </div>
