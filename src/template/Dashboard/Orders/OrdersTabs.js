...
export default function OrdersTabs({ activeTab, onTabChange }) {
  return (
    <div className="flex flex-nowrap overflow-auto justify-between  bg-white dark:bg-dark-box rounded-lg">
      {tabs.map((tab) => (
        <button
          key={tab.id}
          onClick={() => onTabChange(tab.id)}
          className={cn(
            "px-4 py-3 text-sm font-medium border-b-2 transition-all flex-none lg:flex-1",
            activeTab === tab.id
              ? "bg-primary-50 border-primary-500 dark:bg-dark-blue dark:border-dark-title dark:text-dark-title text-primary-500"
