...
export default function EditShippingMethodPage() {
  ...
  useEffect(() => {
    ...
    const fetchMethod = async () => {
      try {
        setFetching(true);
        const response = await shippingService.getMethodById(id);
        ...
        if (data) {
          ...
          toast.error("روش ارسال یافت نشد");
        }
      } catch (error) {
        toast.error(error?.message || "خطا در دریافت اطلاعات روش ارسال");
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await shippingService.updateMethod(id, payload);
      if (response?.success !== false) {
        toast.success("روش ارسال با موفقیت به‌روزرسانی شد");
        router.push("/admin/shipping/methods");
      } else {
        toast.error(response?.message || "خطا در به‌روزرسانی روش ارسال");
      }
    } catch (error) {
  ...
  if (fetching) {
    return (
      <div className="max-w-4xl mx-auto space-y-6">
        <AdminPageHeader title="ویرایش روش ارسال" subtitle="در حال بارگذاری..." icon={Truck} />
        <div className="p-12 flex flex-col items-center justify-center text-gray-400 gap-3 rounded-2xl border border-gray-700/60 bg-gray-800/40">
          <Spinner size="lg" />
          <span>در حال بارگذاری اطلاعات روش ارسال...</span>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6">
      <AdminPageHeader
        ...
        subtitle="تغییر اطلاعات روش حمل و نقل"
        icon={Truck}
        actions={
          <Link
